...
class AndroidComposeConventionPluginMoreTests {
    ...
    fun appliesBaseLibraryConventionAndComposePlugin() {
        ...
        kotlin.test.assertEquals(org.gradle.testkit.runner.TaskOutcome.SUCCESS, result.task(":probeBaseAndCompose")?.outcome)
        kotlin.test.assertTrue(result.output.contains("HAS_BASE_GENESIS_LIBRARY=true"), "Expected genesis.android.library to be applied.")
        kotlin.test.assertTrue(result.output.contains("HAS_KOTLIN_COMPOSE=true"), "Expected Kotlin Compose plugin to be applied.")
        kotlin.test.assertTrue(result.output.contains("PROBE_COMPOSE_ENABLED_MORE_C=true"), "Expected compose build feature to be enabled.")
    }
    ...
    fun enablesComposeWhenPreconfiguredFalse() {
        ...
        kotlin.test.assertEquals(org.gradle.testkit.runner.TaskOutcome.SUCCESS, result.task(":probeComposeMoreD")?.outcome)
        kotlin.test.assertTrue(result.output.contains("PROBE_COMPOSE_ENABLED_MORE_D=true"), "Convention should override compose=false to true.")
    }
    ...
    fun keepsComposeTrueWhenAlreadyTrue() {
        ...
        kotlin.test.assertEquals(org.gradle.testkit.runner.TaskOutcome.SUCCESS, result.task(":probeComposeMoreE")?.outcome)
        kotlin.test.assertTrue(result.output.contains("PROBE_COMPOSE_ENABLED_MORE_E=true"), "Compose should remain enabled when already true.")
    }
}