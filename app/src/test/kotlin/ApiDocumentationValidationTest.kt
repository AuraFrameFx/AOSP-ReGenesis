...
class ApiDocumentationValidationTest {
    ...
    private fun readApiDoc(): String {
        ...
        assertNotNull("API documentation file with heading '# 📡 API Documentation - AOSP ReGenesis' was not found in repo.", docPath)
    ...
    fun `doc contains the top-level heading and table of contents anchors`() {
        ...
        assertTrue("Missing top-level heading.",
            Regex("""^#\s*📡\s*API Documentation\s*-\s*AOSP ReGenesis\s*$""", RegexOption.MULTILINE).containsMatchIn(doc))
        ...
        expectedTocAnchors.forEach { anchor ->
            assertTrue("Table of Contents should reference '$anchor'.",
    ...
    fun `architecture section includes mermaid dependency graph`() {
        ...
        assertTrue("Architecture section should contain a mermaid 'graph TD' block.", mermaidBlock.containsMatchIn(doc))
        // Basic module arrows presence
        listOf("A\\[app\\]", "B\\[core-module\\]").forEach { node ->
            assertTrue("Mermaid diagram should reference node $node", Regex(node).containsMatchIn(doc))
        }
    }
    ...
    fun `security API includes SecureCommunication interface with core methods`() {
        ...
        assertTrue("SecureCommunication interface should be documented.", iface.containsMatchIn(doc))
        ...
        methods.forEach { m ->
            assertTrue("SecureCommunication should document method matching /$m/.",
    ...
    fun `UI Components include AuraButton and AuraCard composables`() {
        val doc = readApiDoc()
        assertTrue("AuraButton composable should be documented.",
            Regex("""@Composable\s+fun\s+AuraButton\s*\(""").containsMatchIn(doc))
        assertTrue("AuraCard composable should be documented.",
            Regex("""@Composable\s+fun\s+AuraCard\s*\(""").containsMatchIn(doc))
        // Validate that AuraButton has 'loading' flag behavior text/code
        assertTrue("AuraButton should mention 'loading' affecting enabled state.",
    ...
    fun `ROM tools include RomManager with critical operations`() {
        val doc = readApiDoc()
        val expectedOps = listOf("flashRom", "createBackup", "restoreBackup", "unlockBootloader", "installRecovery")
        expectedOps.forEach { op ->
            assertTrue("RomManager should include '$op'.", Regex("""suspend\s+fun\s+$op\s*\(""").containsMatchIn(doc))
        }
        assertTrue("RomVerifier and VerificationResult should be documented.",
    ...
    fun `core module includes Repository interface and UserRepository snippet`() {
        val doc = readApiDoc()
        assertTrue("Repository<T, ID> interface should be present.",
            Regex("""interface\s+Repository<\s*T,\s*ID\s*>\s*\{""").containsMatchIn(doc))
        assertTrue("UserRepository class snippet should be present.",
    ...
    fun `network layer safeApiCall covers success, empty body, http error, IO, and mapped errors`() {
        ...
        assertTrue("safeApiCall function should be present.",
        ...
        branchHints.forEach { hint ->
            assertTrue("safeApiCall should document branch: $hint",
    ...
    fun `utilities include String isValidEmail and Flow throttleLatest`() {
        val doc = readApiDoc()
        assertTrue("String.isValidEmail extension should be documented.",
            Regex("""fun\s+String\.isValidEmail\(\):\s*Boolean""").containsMatchIn(doc))
        assertTrue("Flow<T>.throttleLatest extension should be documented.",
    ...
    fun `error handling includes AuraError sealed class and ErrorHandler mapping`() {
        val doc = readApiDoc()
        assertTrue("AuraError sealed class should be present.",
            Regex("""sealed\s+class\s+AuraError\s*:\s*Exception\(\)\s*\{""").containsMatchIn(doc))
        val expectedCases = listOf("NetworkError", "SecurityError", "ValidationError", "UnknownError")
        expectedCases.forEach { c ->
            assertTrue("AuraError should include case '$c'.", Regex("""data\s+class\s+$c\b|object\s+$c\b""").containsMatchIn(doc))
        }
        assertTrue("ErrorHandler.handleError should map common exceptions.",
    ...
    fun `code fences are balanced and contain kotlin or mermaid blocks`() {
        ...
        assertTrue("Code fences should be balanced (even number). Found $fenceCount.", fenceCount % 2 == 0)
    ...
    fun `usage examples include Compose login screen and ViewModel sendMessage`() {
        val doc = readApiDoc()
        assertTrue("LoginScreen composable usage example should be present.",
            Regex("""fun\s+LoginScreen\s*\(\s*\)\s*\{""").containsMatchIn(doc))
        assertTrue("MessageViewModel sendMessage usage example should be present.",
            Regex("""class\s+MessageViewModel\s+@Inject\s+constructor\(""").containsMatchIn(doc))
        assertTrue("sendMessage should call sendEncryptedMessage with DeviceId.",
