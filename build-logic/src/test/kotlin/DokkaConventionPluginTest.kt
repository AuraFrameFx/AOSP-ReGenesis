...
class DokkaConventionPluginTest {
    ...
    fun `applies Dokka plugin when convention plugin is applied`() {
        ...
        assertTrue(project.pluginManager.hasPlugin("org.jetbrains.dokka"), "Dokka plugin should be applied by the convention plugin")
    }
    ...
    fun `registers dokkaHtmlMultiModule task on root project only`() {
        ...
        // Apply plugin to all
        listOf(root, subA, subB).forEach { it.pluginManager.apply(DokkaConventionPlugin::class.java) }
        ...
        assertEquals("documentation", aggregate.group, "Aggregate task should belong to 'documentation' group")
        assertEquals("Generate HTML documentation for all modules", aggregate.description)

        // Subprojects do not have this task
        Assertions.assertNull(subA.tasks.findByName("dokkaHtmlMultiModule"), "Subproject must not register dokkaHtmlMultiModule")
        Assertions.assertNull(subB.tasks.findByName("dokkaHtmlMultiModule"), "Subproject must not register dokkaHtmlMultiModule")
    }
    ...
    fun `aggregate task depends on each subproject's dokkaHtml task`() {
        ...
        assertTrue(deps.containsAll(setOf(":alpha:dokkaHtml", ":beta:dokkaHtml")),
...
class DokkaConventionPluginFunctionalTest {
    ...
    fun setup() {
        ...
        File(testDir, "libA/build.gradle.kts").writeText("""plugins { id("org.jetbrains.kotlin.jvm") }""")
        File(testDir, "libB/build.gradle.kts").writeText("""plugins { id("org.jetbrains.kotlin.jvm") }""")
    }
    ...
    fun `dokkaHtmlMultiModule task is listed and has correct group and description`() {
        ...
        assertTrue(out.contains("dokkaHtmlMultiModule"),
