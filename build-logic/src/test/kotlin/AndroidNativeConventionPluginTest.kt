...
class AndroidNativeConventionPluginTest {
    ...
    fun applies_and_registers_verifyNativeConfig_task_with_metadata() {
        ...
        Assertions.assertEquals("Verify native build configuration", task.description, "verifyNativeConfig description mismatch")
    }
    ...
    fun cleanGeneratedSources_deletes_convention_build_dirs(@TempDir tempDir: File) {
        ...
        Assertions.assertNotNull(clean, "cleanGeneratedSources should be present (from stub plugin)")
        ...
        targets.forEach { rel ->
            val dir = File(buildDir, rel)
            Assertions.assertFalse(dir.exists(), "Expected ${dir.path} to be deleted by cleanGeneratedSources")
        }
    }
