...
class KoverConventionPluginTest {
    ...
    fun staticSourceAssertions() {
        ...
        assertTrue(src.contains("""pluginManager.apply("org.jetbrains.kotlinx.kover")"""),
            "Should apply 'org.jetbrains.kotlinx.kover' plugin")
        assertTrue(Regex("""total\s*\{\s*html\s*\{[^}]*onCheck\s*=\s*false""", RegexOption.IGNORE_CASE or RegexOption.DOT_MATCHES_ALL).containsMatchIn(src),
            "HTML report should have onCheck = false")
        assertTrue(Regex("""total\s*\{\s*xml\s*\{[^}]*onCheck\s*=\s*false""", RegexOption.IGNORE_CASE or RegexOption.DOT_MATCHES_ALL).containsMatchIn(src),
            "XML report should have onCheck = false")
        assertTrue(src.contains("""htmlDir = layout.buildDirectory.dir("reports/kover/html")"""),
            "HTML report directory should be 'build/reports/kover/html'")
        assertTrue(src.contains("""xmlFile = layout.buildDirectory.file("reports/kover/coverage.xml")"""),
            "XML report file should be 'build/reports/kover/coverage.xml'")
        assertTrue(Regex("""verify\s*\{\s*onCheck\s*=\s*true""", RegexOption.IGNORE_CASE or RegexOption.DOT_MATCHES_ALL).containsMatchIn(src),
            "Verify should have onCheck = true")
        assertTrue(Regex("""rule\s*\{\s*minBound\(\s*80\s*\)""", RegexOption.IGNORE_CASE or RegexOption.DOT_MATCHES_ALL).containsMatchIn(src),
    ...
    fun pluginTypeContract() {
        ...
        assertTrue(Plugin::class.java.isAssignableFrom(clazz), "Class must implement Plugin<Project>")
    }
    ...
    fun applyPluginRegistersKoverExtension() {
        ...
        try {
            ...
            org.junit.jupiter.api.Assumptions.assumeTrue(false, "Skipping: Kover plugin not available in test environment (${ex::class.java.simpleName}: ${ex.message})")
        }

        assertTrue(project.pluginManager.hasPlugin("org.jetbrains.kotlinx.kover"),
            "Expected 'org.jetbrains.kotlinx.kover' to be applied")

        val extContainer = project.extensions
        val koverExt = try {
            val clazz = Class.forName("kotlinx.kover.gradle.plugin.dsl.KoverProjectExtension") as Class<Any>
